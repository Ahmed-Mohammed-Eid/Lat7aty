...
const ItemFormEdit = ({item: itemFromServer, id}) => {
    // STATES
    ...
    const handleSubmit = (e) => {
        ...
        // Append colors to FormData object
        for (let i = 0; i < item.colors.length; i++) {
            formData.append("colors", item.colors[i]);
        }

        // Append sizes to FormData object
        ...
        let url = "https://api.lathaty.com/api/v1/edit/farm/item";

        if (role === 'farmer') {
            url = "https://api.lathaty.com/api/v1/edit/farmer/item";
        }
    ...
    // EFFECT TO SET THE ITEM
    useEffect(() => {
        if (itemFromServer) {
            setItem({
                itemName: itemFromServer.itemName,
                pricingSystem: itemFromServer.pricingSystem,
                itemPrice: itemFromServer.itemPrice,
                files: [],
                sizes: [],
                colors: [],
                farmId: itemFromServer.farmId,
                itemBlocked: itemFromServer.itemBlocked,
            });
        }
    }, [itemFromServer]);
    ...
    return (
        <>
            <form onSubmit={handleSubmit} className={"col-12"}>
                <div className="card">
                    <h1 className="text-2xl mb-5 uppercase">Edit Item</h1>
                ...
                <ItemOptions
                    ...
                    selectedColors={itemFromServer?.colors}
                />
                ...
                <ItemSizes
                    ...
                    selectedSizes={itemFromServer?.sizes}
                />
