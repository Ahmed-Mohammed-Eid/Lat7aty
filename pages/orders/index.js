...
const OrdersTable = () => {
        ...
        if (token) {
            ...
            axios
                ...
                .then((res) => {
                    ...
                    const index = ordersCopy.findIndex(order => order._id === selectedOrderToDelete._id);
    ...
    function getAvailableCouriers(orderId) {
        ...
        // GET COURIERS
        axios.get(`https://api.lathaty.com/api/v1/available/couriers`, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
            params: {
    ...
    function assignOrderHandler() {
        ...
        if (token) {
            ...
            socket.on("courier_assigned", (data) => {
                ...
                const index = ordersCopy.findIndex(order => order._id === orderId);
                // GET THE ORDER
    ...
    return (
        <div className="card">
            ...
            >
                ...
                <Column
                    ...
                    body={(rowData) => {
                        ...
                        return (
                            <span className={rowData.orderType === "instant" ? "text-green-500" : "text-red-500"}>
                                {rowData.orderType === 'instant' ? 'Instant' : 'Postponed'}
                            </span>
                        );
                    }}
                />
                    ...
                    body={(rowData) => {
                        ...
                        if (rowData?.orderStatus.length > 0) {
                            ...
                            if (lastStatus === "pending") {
                                ...
                                return <span className="px-3 py-1 text-white rounded-full bg-red-500 capitalize"
                                             style={{fontSize: '11px'}}>{lastStatus}</span>
                            } else if (lastStatus === "canceled") {
                                return <span className="px-3 py-1 text-white rounded-full bg-red-500 capitalize"
                                             style={{fontSize: '11px'}}>{lastStatus}</span>
                            } else if (lastStatus === "transporting") {
                                return <span className="px-3 py-1 text-white rounded-full bg-purple-500 capitalize"
                                             style={{fontSize: '11px'}}>{lastStatus}</span>
                        ...
                        return (
                            <div className="flex gap-2">
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        router.push(`/orders/edit/${rowData._id}`);
                                    }}
                                >
                                    Edit
                                </button>
            ...
            >
                <div className="grid col-12">
                    ...
                    <div className="col-6">
                        ...
                        <div>{`${moment(selectedOrder?.createdAt).format("DD/MM/YYYY")} - (${moment(selectedOrder?.orderDate).format("DD/MM/YYYY")})`}</div>
                    </div>
                    <div className="col-6">
                        <div className="font-bold">ORDER TIME</div>
                        <div>{selectedOrder?.orderTime}</div>
                    </div>
                    {selectedOrder?.courierId && (<div className="col-12 card grid mt-4">
                        <div className={'col-6'}>
                            <div className="font-bold">COURIER NAME</div>
                            <div>{selectedOrder?.courierId?.courierName}</div>
                        </div>
                        <div className={'col-6'}>
                            <div className="font-bold">COURIER PHONE</div>
                            <div>{selectedOrder?.courierId?.phoneNumber}</div>
                        </div>
                        <div className={'col-6'}>
                            <div className="font-bold">COURIER COMPANY</div>
                            <div>{selectedOrder?.courierId?.companyName}</div>
                        </div>
                        <div className={'col-6'}>
                            <div className="font-bold">COURIER CAR</div>
                            <div>{selectedOrder?.courierId?.carBrand} - {selectedOrder?.courierId?.carModel} - {selectedOrder?.courierId?.licenseNumber}</div>
                        </div>
                        <div className={'col-6'}>
                    ...
                    {selectedOrder?.orderItems?.length > 0 && (<div className="col-12 card mt-3">
                        ...
                        <div className="grid col-12 mt-2">
                            <DataTable
                                ...
                                paginator
                                rows={5}
                                totalRecords={selectedOrder?.orderItems?.length}
                                lazy
                                first={1}
                            >
                                <Column
                    ...
                    {selectedOrder?.notes && (<div className="col-12 card mt-3">
                        <div className="font-bold">ORDER NOTES</div>
                        <div>{selectedOrder?.notes}</div>
                    </div>)}
                    {(selectedOrder?.attachments.length > 0) && (<div className="col-12 card">
                ...
                footer={
                    (
                        <div>
                            ...
                            <Button
                                icon="pi pi-check"
                                onClick={() => deleteOrderHandler(selectedOrderToDelete)}
                                style={{
            ...
            <Dialog
                header="Assign Order"
                onHide={() => setSelectedOrderToAssign({
                    orderId: null,
                    courierId: null,
                })}
                ...
                <div className="grid col-12">
                    <div className="col-12">
                        <label className="font-bold mb-1 block" htmlFor={"SelectACourier"}>Courier:</label>
                        <Dropdown
                    ...
                    <button
                        ...
                        style={{
                            ...
                            marginLeft: "auto",
                            marginRight: ".5rem",
                            backgroundColor: assignLoader ? "#b5e2b5" : "#28a745",
                        }}>
                        {assignLoader ? (
                            <ProgressSpinner
                                strokeWidth="4"
            ...
            <Dialog onHide={() => setSelectedOrderImage(null)} visible={selectedOrderImage} style={{width: "90vw", maxWidth: "600px"}}>
                <Image src={selectedOrderImage || ''} alt="Order Image" width={550} height={500} style={{
                    objectFit: 'contain',
                    display: 'block',
                    marginLeft: 'auto',
                }}/>
